...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const isOwner = ownerNumber == sender ? true : ["6282331660134@s.whatsapp.net", "6282331660134@s.whatsapp.net"].includes(sender) ? true : false
		...
		const sendContact = (jid, numbers, name, quoted, mn) => {
			...
			let vcard = `BEGIN:VCARD\n` // metadata of the contact card
				...
				`VERSION:3.0\n` +
				'N:;SkylarKaf.;;;' +
				'FN:Andik Febriansyah.\n' // full name
				+
				`ORG:${ownerName};\n` // the organization of the contact
				+
				`item1.TEL;type=CELL;type=VOICE;waid=6282331660134:+62 823-3166-0134\n` // WhatsApp ID + phone number
				+
				`item1.X-ABLabel:Andik Febriansyah\n` +
				`item2.EMAIL;type=INTERNET:skylarkaf7@gmail.com\n` +
				`item2.X-ABLabel:Email\n` +
				`item3.URL:https://skylarkaf.github.io\n` +
				`item3.X-ABLabel:web\n` +
				`item4.URL:https://github.com/SkylarKaf\n` +
				`item4.X-ABLabel:My Github\n` +				
				`item5.ADR:;;Region;;;;\n` +
				`item5.X-ABLabel:Federasi Isekai\n` +			
				`item6.X-ABLabel:i'm just a student, still learning programming\n`		
				+
				`END:VCARD`
